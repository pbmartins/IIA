...
class SearchNode:
    ...
    def __repr__(self):
        return str(self)
    def is_descedent(self, parent_state):
        if self.parent == None:
            return False
        elif self.parent.state == parent_state:
            return True
        return self.parent.is_descedent(parent_state)
        ...
        while self.open_nodes != []:
            ...
            if self.strategy != 'depth' or node.depth < self.boundary:
                for a in self.problem.domain.actions(node.state):
                    ...
                    if not node.is_descedent(newstate):
                        lnewnodes += [SearchNode(newstate,node, node.cost + \
                                self.problem.domain.cost(newstate, a), node.depth + 1, \
